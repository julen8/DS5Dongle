...
static inline void __not_in_flash_func(l2capHandleInterruptDataPacket)(uint8_t* packet, const uint16_t size) {
    ...
    // 静默检测
    if (atomic_load_explicit(&config.micActive, memory_order_relaxed) || ds5StateUnion->packet.LeftStickX < 120 || ds5StateUnion->packet.LeftStickX > 140 || ds5StateUnion->packet.LeftStickY < 120 ||
        ds5StateUnion->packet.LeftStickY > 140 || ds5StateUnion->packet.RightStickX < 120 || ds5StateUnion->packet.RightStickX > 140 || ds5StateUnion->packet.RightStickY < 120 ||
        ds5StateUnion->packet.RightStickY > 140 || ds5StateUnion->packet.TriggerLeft > 0 || ds5StateUnion->packet.TriggerRight > 0 || ds5StateUnion->data[7] != DirectionNone ||
        ds5StateUnion->data[8] != 0x00 || ds5StateUnion->data[9] != 0x00) {
